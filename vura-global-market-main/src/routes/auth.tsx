...
function AuthPage() {
  ...
  useEffect(() => { if (user) navigate({ to: "/account" }); }, [user]);

  const google = async () => {
    setBusy(true);
    const result = await lovable.auth.signInWithOAuth("google", { redirect_uri: window.location.origin + "/account" });
    if (result.error) { toast.error("Sign-in failed"); setBusy(false); return; }
    ...
    <main className="mx-auto max-w-md px-4 py-8 md:py-16">
      <div className="rounded-3xl bg-card p-6 md:p-8 shadow-card">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">{t("welcomeDesc")}</p>
        </div>

        <Button onClick={google} disabled={busy} variant="outline" className="w-full h-12 font-semibold">
        ...
        <div className="space-y-3">
          ...
          <Button onClick={emailAuth} disabled={busy || !email || !password} className="w-full bg-primary text-primary-foreground hover:bg-primary/90 h-11">
            {mode === "signin" ? t("signIn") : "Sign up"}
          </Button>
      ...
      <div className="text-center mt-4">
        <Link to="/" className="text-xs text-muted-foreground hover:text-foreground">{t("backHome")}</Link>
      </div>
    </main>
  );
}
